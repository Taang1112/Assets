...
<div class="card">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div>
            <h6 class="mb-0 fw-bold fs-6"><i class="bi bi-tags me-2 text-primary"></i>Daftar Kategori Produk</h6>
            <small class="text-muted">Kelola pengelompokan produk toko</small>
        </div>
        <a href="{{ route('kategori-produk.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>Tambah Kategori</a>
    </div>
    ...
    <div class="px-4 py-3 border-bottom bg-light bg-opacity-50">
        <form method="GET" class="row g-2 align-items-center">
            ...
            <div class="col-auto d-flex gap-2">
                ...
                @if($search)
                    <a href="{{ route('kategori-produk.index') }}" class="btn btn-outline-secondary"><i class="bi bi-x-circle me-1"></i>Reset</a>
                @endif
            </div>
        </form>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tr>
                ...
                <td><span class="badge bg-secondary-subtle text-secondary px-2.5 py-1 fw-bold">{{ $item->produk_count }} produk</span></td>
                ...
                <td class="text-center pe-4">
                    <div class="d-flex justify-content-center gap-1">
                        <a href="{{ route('kategori-produk.show', $item) }}" class="btn btn-icon btn-outline-secondary" title="Detail"><i class="bi bi-eye"></i></a>
                        <a href="{{ route('kategori-produk.edit', $item) }}" class="btn btn-icon btn-outline-primary" title="Edit"><i class="bi bi-pencil"></i></a>
                        <button class="btn btn-icon btn-outline-danger" data-bs-toggle="modal" data-bs-target="#modalHapus" data-id="{{ $item->kategori_produk_id }}" data-nama="{{ $item->nama_kategori }}" title="Hapus"><i class="bi bi-trash"></i></button>
                    </div>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="7">
                    <div class="empty-state">
                        ...
                        <h6 class="fw-bold mb-1">Belum Ada Kategori Produk</h6>
                        <p class="text-muted small mb-3">Data kategori tidak ditemukan atau belum pernah ditambahkan.</p>
                        <a href="{{ route('kategori-produk.create') }}" class="btn btn-primary btn-sm px-4"><i class="bi bi-plus-lg me-1"></i>Tambah Kategori Pertama</a>
                    </div>
                </td>
            </tr>
            @endforelse
