...
<div class="row justify-content-center"><div class="col-lg-9">
    <nav aria-label="breadcrumb" class="mb-3"><ol class="breadcrumb mb-0"><li class="breadcrumb-item"><a href="{{ route('transaksi.index') }}" class="text-decoration-none text-primary">Transaksi</a></li><li class="breadcrumb-item active">Edit</li></ol></nav>
    <div class="card"><div class="card-header"><h6 class="mb-0 fw-bold">Edit Transaksi: {{ $transaksi->kode_transaksi }}</h6></div>
    <div class="card-body p-4">
        <form action="{{ route('transaksi.update', $transaksi) }}" method="POST" novalidate>
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-sm-6">
                    <label class="form-label">Kode Transaksi <span class="text-danger">*</span></label>
                    <input type="text" name="kode_transaksi" class="form-control @error('kode_transaksi') is-invalid @enderror" value="{{ old('kode_transaksi', $transaksi->kode_transaksi) }}" required>
                ...
                <div class="col-sm-6">
                    ...
                    <input type="datetime-local" name="tanggal_transaksi" class="form-control @error('tanggal_transaksi') is-invalid @enderror" value="{{ old('tanggal_transaksi', $transaksi->tanggal_transaksi->format('Y-m-d\TH:i')) }}" required>
                ...
                <div class="col-sm-4">
                    ...
                    <select name="pelanggan_id" class="form-select @error('pelanggan_id') is-invalid @enderror" required>
                        ...
                        @foreach($pelanggan as $p)
                            <option value="{{ $p->pelanggan_id }}" {{ old('pelanggan_id', $transaksi->pelanggan_id) == $p->pelanggan_id ? 'selected' : '' }}>{{ $p->nama_pelanggan }} ({{ $p->kode_pelanggan }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-4">
                    ...
                    <select name="karyawan_id" class="form-select @error('karyawan_id') is-invalid @enderror" required>
                        ...
                        @foreach($karyawan as $k)
                            <option value="{{ $k->karyawan_id }}" {{ old('karyawan_id', $transaksi->karyawan_id) == $k->karyawan_id ? 'selected' : '' }}>{{ $k->nama_lengkap }} ({{ $k->nik }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-6">
                    ...
                    <select name="metode_pembayaran" class="form-select @error('metode_pembayaran') is-invalid @enderror" required>
                        @foreach(['Cash','Transfer','QRIS','E-Wallet'] as $m)
                            <option value="{{ $m }}" {{ old('metode_pembayaran', $transaksi->metode_pembayaran) === $m ? 'selected' : '' }}>{{ $m }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-6">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        @foreach(['Pending','Selesai','Batal'] as $s)
                            <option value="{{ $s }}" {{ old('status', $transaksi->status) === $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
            ...
            <div class="d-flex justify-content-end gap-2">
                <a href="{{ route('transaksi.show', $transaksi) }}" class="btn btn-outline-secondary px-4">Batal</a>
